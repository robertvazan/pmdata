...
public class ImageViewer {
    ...
    private static Dimension size(byte[] image) {
        return Exceptions.silence().get(Exceptions.sneak().supplier(() -> {
            switch (mime(image)) {
                case "image/png":
                case "image/jpeg":
     ...
     */
    public static void view(byte[] image) { new ImageViewer().image(image).render(); }
    public static void view(int scale, byte[] image) { new ImageViewer().scale(scale).image(image).render(); }
    public static void view(String title, byte[] image) { new ImageViewer().title(title).image(image).render(); }
    public static void view(String title, int scale, byte[] image) { new ImageViewer().title(title).scale(scale).image(image).render(); }
}