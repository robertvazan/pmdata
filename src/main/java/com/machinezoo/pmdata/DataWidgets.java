...
import com.machinezoo.foxcache.*;
...
public class DataWidgets {
    ...
    @CodeIssue("Cache ignores changes in XML attributes / method parameters.") private static SiteFragment assemble(SiteFragment.Key key, Runnable runnable) {
    ...
    private static final Cache<SiteFragment.Key, ReactiveLazy<SiteFragment>> outerCache = CacheBuilder.newBuilder()
        ...
        .maximumSize(100)
        .build();
    @DraftCode("configurable cache") private static void execute(Runnable runnable) {
    ...
    private static class WidgetMethod {
        ...
        Runnable runnable(Object object) {
            ...
            return () -> execute(
                Exceptions.wrap().runnable(() -> consumer.accept(method.invoke(object, suppliers.stream().map(p -> p.get()).toArray(Object[]::new)))));
        }
    }
    ...
    private static synchronized List<WidgetMethod> widgets(Class<?> clazz) { return cache.computeIfAbsent(clazz, DataWidgets::compile); }
